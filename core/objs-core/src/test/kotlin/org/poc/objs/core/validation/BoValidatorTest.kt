...
class BoValidatorTest {
    ...
    fun shouldAcceptValidEntity() {
        val entity = BoEntity(type = "Person", version = "1", payload = mutableMapOf("name" to "Ada"))
        assertThat(validator.validateEntities(listOf(entity)).isValid).isTrue()
    }

    @Test
    fun shouldRejectInvalidEntityPayload() {
        val entity = BoEntity(type = "Person", version = "1", payload = mutableMapOf())
    ...
    fun shouldReportMissingEndpoint() {
        val edge = BoEdge(source = UuidV7.generate(), target = UuidV7.generate(), role = "knows")
...
class BoPersistGateTest {
    ...
    fun shouldFailFastOnInvalidEntitiesBeforeEdges() {
        ...
        val graph = BoGraph(
            entities = mutableListOf(BoEntity(type = "Person", version = "1", payload = mutableMapOf())),
            edges = mutableListOf(),
        )
    ...
    fun shouldValidateEdgeAgainstStoreEntity() {
        ...
        val neu = UuidV7.generate()
        val graph = BoGraph(
            entities = mutableListOf(
                BoEntity(id = neu, type = "Person", version = "1", payload = mutableMapOf("name" to "Bob")),
            ),
    ...
    fun shouldAssignIdsWhenMissing() {
        ...
        val graph = BoGraph(
            entities = mutableListOf(
                BoEntity(type = "Person", version = "1", payload = mutableMapOf("name" to "Ada")),
            ),
        )
        assertThat(gate.validateWrite(graph).isValid).isTrue()
        assertThat(graph.entities[0].id).isNotNull()
        assertThat(graph.entities[0].id!!.version()).isEqualTo(7)
    }
}
