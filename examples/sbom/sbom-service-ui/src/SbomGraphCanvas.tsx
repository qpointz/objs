...
function CanvasInner(
  ...
  }, [visible, skeleton.nodes, skeleton.edges, setEdges, setNodes])
  ...
  useEffect(() => {
    ...
    setEdges((curr) =>
      curr.map((e) => {
        const selected = e.source === selectedAssetId || e.target === selectedAssetId
        ...
        const stroke = selected ? EDGE_SELECTED : draftStatusColor(status) ?? EDGE_COLOR
        const opacity = selected ? 1 : dimmed ? DIMMED_OPACITY : 1
        return {
          ...e,
          animated: selected,
          style: {
            ...e.style,
            stroke,
            strokeWidth: selected || status !== 'unchanged' ? 3 : 2,
            strokeDasharray: status === 'deleted' ? '6 4' : undefined,
            opacity,
          },
          markerEnd: {
  ...
  }, [visible, selectedAssetId, setEdges, setNodes])
  ...
  return (
    <ReactFlow
      ...
      onPaneClick={() => onSelectAsset(null)}
      onInit={() => applyLayout()}
