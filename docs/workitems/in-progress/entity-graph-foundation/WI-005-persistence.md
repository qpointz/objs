...
**Gaps:** G-5 UUID, G-9 same gate, G-10 Flyway, G-11 **H2** tests
...
- Relational mapping for entities (generic attributes + **UUID** PK + JSONB payload + annotations JSON)
- Mapping for edges (**source** / **target** UUID FKs, role, properties JSONB; edge UUID id preferred)
...
- [ ] No-id items created with `UUID.randomUUID()`; id-present items update existing rows; unknown id rejects
