...
**Status:** sketch (refine in WI-002 / WI-003)  
**Rule:** End-user UI and domain API **must not** expose terms from this document (graph, entity, edge, matcher, pool, annotation, …).

This document maps locked product requirements to **hybrid** storage (SBOM domain tables + objs graph) and proposes how to **read/write** each capability. Gaps are flagged for WI-003.

**Personas (UI only):** Applications tab (J1–J2) vs Portfolios tab (J3 taxonomy + MI). MI selection pipeline: R21 → R22 → graph-id-set matcher → Gremlin.
...
| **Application version** | **SBOM entity** + **`graph_id` → separate named graph** | Created from draft (copy); historical BOM |
...
| R22 | Application set → latest version graphs | For each app id: pick latest `sbom_application_version.graph_id`; skip apps with no version | DOMAIN **OK** |
...
## 5. Domain service boundaries (example module)
...
**Compile object model (G-A8):** the application’s typed/domain classes for assets are **generated/compiled from the graph schema** (user will generate schema artefacts). Runtime forms (Journey 2 advanced search) and validation should follow the same schema — not a divergent hand model.

Still may need **additive** schema/allow-list changes (prefer extending the existing catalog):

- Owning-application: annotation **`owner`** = app **name** (G-P5) — already locked  
- Friendly relation labels: **beautifier** of role names + optional override map (G-P6) — locked  
...
Breaking changes in `objs-sbom-example` packaging are allowed; **prefer evolving the existing schema** over replacing it wholesale.
...
## 8. Open decisions feeding WI-002

1. Exact SBOM DDL (application / draft / version / portfolio tree; one draft per app vs many)  
2. G-F1 / G-F2 — in-story foundation WIs vs deferred after stopgap  
3. Codegen/compile toolchain details once user-generated schema artefacts land (G-A8)  
4. ~~Portfolio pin version?~~ **Locked:** applications only  
5. ~~App→app encoding?~~ **Locked (G-P4):** inferred from shared objects  
6. ~~Ownership storage?~~ **Locked (G-P5):** annotation `owner` = application name  
