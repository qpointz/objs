...
| G-A6 | Hybrid persistence | **resolved** | SBOM tables for application, **draft**, **version**, **portfolio**. Draft/version each → own objs named graph. Portfolios domain-only. Assets + relations only in graphs |
...
| G-P2 | Draft vs versions | **resolved** | SBOM entities: **edit draft** and **app version**; each has own `graph_id` |
...
| G-F5 | Version snapshot semantics | **resolved** | New version entity + new graph; copy from draft |
| G-F6 | Cross-version compare | **deferred** | Nice-to-have |
| G-F7 / **FB-4** | MI over selection (Gremlin) | **open** | Prefer Gremlin over selected graphs; see FOUNDATION-BACKLOG |
| G-F8 / **FB-5** | Graph-id-set matcher (portfolio → graphs) | **open** | Explicit multi-graph selection; see FOUNDATION-BACKLOG |

## Process

| #    | Topic                   | Status       | Resolution                                   |
| ---- | ----------------------- | ------------ | -------------------------------------------- |
| G-X1 | Example packaging vs `objs-app` | **resolved** | See resolved lock — `examples/sbom/{sbom-service,sbom-service-ui}`; `objs-app` is foundation-only |
...
**Status:** open — tracked as **[FB-4](FOUNDATION-BACKLOG.md)**.  
**Needed by:** All portfolio MI reports (MI-1…MI-4).

**Product lock:** Reports run as **Gremlin traverse** over the multi-graph selection from FB-5 (not ad-hoc domain folds as the primary design). Domain maps results to product DTOs.

**Propose (default):** `objs-gremlin-core` `selectAndEval` (or equivalent programmatic API) with report scripts. Domain-only fold over `selectInGraph` is a **stopgap** only if WI-003 defers FB-4.

**Questions:**

1. Cap / warn when selected level covers too many apps (graph union size)?  
2. Any hard SLA for MI run time?

**Blocks:** WI-013; WI-003.

---

### G-F8 / FB-5 — Graph-id-set matcher (portfolio → graphs)

**Status:** open — tracked as **[FB-5](FOUNDATION-BACKLOG.md)**.  
...
**Propose (default):** Add matcher such as `graphs-in: [uuid…]` (name TBD) in objs-core; wire into store select + gremlin materialization. Until then, stopgap = N× select / manual union — record debt.

**Questions:**

1. Cap application set size for v1 (e.g. warn above N apps under the selected node)?  
2. In-story foundation WI in WI-003 vs parked with stopgap?
...
examples/
  sbom/
    sbom-service/       # domain + Boot-launchable app (objs-core, objs-gremlin-core; no objs-service REST)
...
2. **`objs-app`** remains the **foundation** runnable (objs-service + workbench + gremlin REST). It does **not** depend on `:sbom-service`.  
3. **`:sbom-service`** is the inventory **core + launchable** application (`bootRun` / `run`).  
4. **`:sbom-service-ui`** mirrors `:objs-service-ui` build (node-gradle, Vite → `processResources` → `static/ui`). Consumed as `runtimeOnly` by `:sbom-service`.  
5. Full rework of former `objs-sbom-example` allowed; no backward compatibility.  
6. No `objs-service` REST / workbench on the SBOM product path.
