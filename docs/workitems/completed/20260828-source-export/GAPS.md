...
| ID | Topic | Status | Notes |
|----|-------|--------|-------|
| G-1 | Non-package `objs` identifiers | **deferred** | v1 keeps REST paths, UI mounts, config prefixes, and `OBJS_*` environment names; destination-specific overrides may be added later |
| G-2 | `Objs*` class simple names | **deferred** | v1 moves packages only; simple class names remain unchanged |
| G-10 | Module subset export | **deferred** | Export full repo; optional `MODULES=` flag future |
| G-9 | Git in output | **deferred** | Destination `git init` remains a manual step |
