...
| C-3 | Persist central schema catalog (type+version) + allowed-edge rules as PostgreSQL tables | feature | in-progress | [`planned/graph-config-seeds/`](planned/graph-config-seeds/STORY.md) Stage 1 |
| C-4 | Extensible multi-document graph configuration seeds with durable ledger | feature | planned | [`planned/graph-config-seeds/`](planned/graph-config-seeds/STORY.md) Stage 2 |
