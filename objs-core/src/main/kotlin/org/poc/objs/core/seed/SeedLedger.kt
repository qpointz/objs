...
@Service
class SeedLedger(
    private val repository: SeedLedgerRepository,
) {
    fun find(seedKey: String): SeedLedgerRecord? = repository.findById(seedKey).orElse(null)
    ...
    fun recordSuccess(seedKey: String, fingerprint: String) {
        ...
        val record = repository.findById(seedKey).orElseGet {
            SeedLedgerRecord(seedKey = seedKey, createdAt = now)
        }
