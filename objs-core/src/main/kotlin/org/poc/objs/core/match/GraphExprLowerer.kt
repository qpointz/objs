...
/**
 * Lowers `graph-expr` AST to [GraphExprPushdown] when the tree is only equality / inequality
 * (`==` / `===` / `!=` / `!==`) of `id` / `a.*` combined with `&&` / `||`.
 * Anything else → null (local eval).
 */
object GraphExprLowerer : ScriptVisitor() {

    fun toPushdown(compiled: JexlExpression): GraphExprPushdown? {
        ...
        if (groups.isEmpty()) {
            return GraphExprPushdown(dnf = emptyList())
        }
        return GraphExprPushdown(dnf = groups)
    }
    ...
    private fun foldAndGroup(atoms: List<GraphAtom>): GraphExprAndGroup? {
...
/** DNF pushdown plan for [GraphExprMatcher] (`==`/`!=` with `&&`/`||` over `id` / `a.*`). */
data class GraphExprPushdown(
    val dnf: List<GraphExprAndGroup>,
) {
...
/** One AND-group inside a [GraphExprPushdown] DNF. */
data class GraphExprAndGroup(
