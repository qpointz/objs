...
object IdentityProjection {
    fun project(contentSchema: SchemaNode, payload: Map<String, Any?>): Map<String, Any?> {
        require(contentSchema.type == SchemaType.OBJECT) {
     ...
     */
    fun identifierPaths(contentSchema: SchemaNode): Set<String> {
        require(contentSchema.type == SchemaType.OBJECT) {
            "identity projection requires an OBJECT contentSchema"
        }
    ...
    private fun walkIdentifierPaths(
        ...
        for (field in schema.fields.orEmpty()) {
            ...
            when (field.schema.type) {
                SchemaType.OBJECT -> walkIdentifierPaths(field.schema, path, out)
