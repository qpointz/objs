...
class NamedGraphStoreTest {
    ...
    private fun addEdge(graphId: UUID, source: UUID, target: UUID): UUID {
        ...
        assertThat(
            graphStore.write(
                Graph(
                    edges = mutableListOf(
                        Edge(id = edgeId, graphId = graphId, source = source, target = target, role = "knows"),
                    ),
                ),
            ).isValid,
        ).isTrue()
    ...
    fun shouldRoundTripCreateAndGet_preservingIds() {
        val created = namedGraphs.create(
            GraphSpec(annotations = mapOf("pack" to "p1"), entityIds = setOf(a, b)),
        )
    ...
    fun shouldResolveLatestPayload_withoutRewritingMembership() {
        val created = namedGraphs.create(GraphSpec(entityIds = setOf(a, b)))
    ...
    fun shouldRejectEdgeWithoutEndpointMembers() {
        val holder = namedGraphs.create(GraphSpec(entityIds = setOf(a, b)))
        val edgeId = addEdge(holder.id, a, b)

        assertThatThrownBy {
            namedGraphs.create(
                GraphSpec(entityIds = setOf(a), edgeIds = setOf(edgeId)),
            )
    ...
    fun shouldRejectMissingEntityId() {
        ...
        assertThatThrownBy {
            namedGraphs.create(GraphSpec(entityIds = setOf(missing)))
        }.isInstanceOf(GraphException::class.java)
            .extracting("code")
    ...
    fun shouldMergeGraphs_firstSeenOnOverlappingMembersAndEdges() {
        val left = namedGraphs.create(GraphSpec(entityIds = setOf(a, b)))
        addEdge(left.id, a, b)
    ...
    fun shouldMergeGraphs_withCustomPolicyPreferringIncomingEdge() {
        val left = namedGraphs.create(GraphSpec(entityIds = setOf(a, b)))
        addEdge(left.id, a, b)
        val right = namedGraphs.create(GraphSpec(entityIds = setOf(a, b)))
        addEdge(right.id, a, b)
        val incomingWins = object : FirstSeenGraphMergePolicy() {
            override fun onDuplicateEdge(kept: Edge, incoming: Edge): Edge = incoming
        }
    ...
    fun shouldFailMergeGraph_whenAnySourceMissing() {
        val left = namedGraphs.create(GraphSpec(entityIds = setOf(a)))
        val missing = UUID.randomUUID()
    ...
    fun shouldMatchCopyMembership_whenMergingOneSource() {
        val source = namedGraphs.create(GraphSpec(entityIds = setOf(a, b)))
        addEdge(source.id, a, b)
    ...
    fun shouldAttachAndDetachMember() {
        val graph = namedGraphs.create(GraphSpec())
    ...
    fun shouldMutate_upsertingEdge_stampingGraphId() {
        ...
        val result = namedGraphs.mutate(
            graph.id,
            graphMutation {
                edges { set(Edge(source = a, target = b, role = "knows")) }
            },
        )
    ...
    fun shouldAllowMutate_edgeToEntityUpsertedInSameMutation() {
        ...
        val result = namedGraphs.mutate(
            ...
            graphMutation {
                ...
                edges { set(Edge(source = a, target = neu, role = "knows")) }
            },
        )
    ...
    fun shouldMutate_deleteEntityDetachingMembershipOnly_keepingPoolEntity() {
        ...
        val result = namedGraphs.mutate(
            graph.id,
            graphMutation { entities { unset(a) } },
        )
    ...
    fun shouldMutate_deleteEdgeBelongingToThisGraphOnly() {
        val g1 = namedGraphs.create(GraphSpec(entityIds = setOf(a, b)))
        val g2 = namedGraphs.create(GraphSpec(entityIds = setOf(a, b)))
        val edgeInG1 = addEdge(g1.id, a, b)
        val edgeInG2 = addEdge(g2.id, a, b)

        val result = namedGraphs.mutate(
            g1.id,
            graphMutation { edges { unset(edgeInG2) } },
        )
    ...
    fun shouldFailMutate_whenGraphMissing() {
        val missing = UUID.randomUUID()
        assertThatThrownBy {
            namedGraphs.mutate(missing, GraphMutation())
        }.isInstanceOf(GraphException::class.java)
            .extracting("code")
            .isEqualTo("GRAPH_NOT_FOUND")
    }
    ...
    fun shouldReplace_matchingSetExactly_pruningExtras() {
        ...
        val result = namedGraphs.mutate(
            ...
            graphMutation {
                ...
                edges { set(Edge(source = a, target = keep, role = "knows")) }
            },
        )
    ...
    fun shouldReplace_emptySet_clearingContents_keepingGraphId() {
        ...
        val result = namedGraphs.mutate(
            graph.id,
            graphMutation { mode(MutationMode.REPLACE) },
        )
    ...
    fun shouldSearch_returnEmpty_whenNoQAndNoExpr() {
        namedGraphs.create(GraphSpec(annotations = mapOf("env" to "prod"), entityIds = setOf(a)))
    ...
    fun shouldSearch_byAnnotationSubstring_caseInsensitive() {
        val hit = namedGraphs.create(GraphSpec(annotations = mapOf("env" to "Production"), entityIds = setOf(a)))
        namedGraphs.create(GraphSpec(annotations = mapOf("env" to "test"), entityIds = setOf(b)))
    ...
    fun shouldSearch_byUuidPrefix() {
        val hit = namedGraphs.create(GraphSpec(entityIds = setOf(a)))
        namedGraphs.create(GraphSpec(entityIds = setOf(b)))
    ...
    fun shouldSearch_andQWithExpr() {
        val hit = namedGraphs.create(GraphSpec(annotations = mapOf("env" to "prod", "app" to "acme"), entityIds = setOf(a)))
        namedGraphs.create(GraphSpec(annotations = mapOf("env" to "prod", "app" to "other"), entityIds = setOf(b)))
        namedGraphs.create(GraphSpec(annotations = mapOf("env" to "test", "app" to "acme"), entityIds = setOf(a)))
    ...
    fun shouldMatchingHeaders_byAnnotationEquality() {
        val hit = namedGraphs.create(
            GraphSpec(annotations = mapOf("app" to "payments", "appVersion" to "1.0.0"), entityIds = setOf(a)),
        )
        namedGraphs.create(
            GraphSpec(annotations = mapOf("app" to "payments", "appVersion" to "2.0.0"), entityIds = setOf(b)),
        )
        namedGraphs.create(
            GraphSpec(annotations = mapOf("app" to "billing", "appVersion" to "1.0.0"), entityIds = setOf(a)),
        )

        val headers = namedGraphs.matchingHeaders(
            GraphExprMatcher("a.app == 'payments' && a.appVersion == '1.0.0'"),
        )
    ...
    fun shouldSearch_respectLimitAndStableOrder() {
        repeat(5) { i ->
            namedGraphs.create(GraphSpec(annotations = mapOf("tag" to "shared-$i"), entityIds = setOf(a)))
        }
    ...
    fun shouldListGraphIdsAndIncidentEdges_forReverseLookup() {
        ...
        graphStore.write(
            Graph(
                entities = mutableListOf(
                    Entity(id = orphan, type = "Person", schemaVersion = "1", payload = mutableMapOf("name" to "O")),
                ),
            ),
        )
    ...
    fun shouldSelectFromPool_withCompareAndPrefixPushdown() {
        ...
        val prefix =
            graphStore.selectFromPool(
                ObjExprMatcher("type == 'Person' && p.name =~ '^Bo'"),
            )
        assertThat(prefix.entities.map { it.payload["name"] }).containsExactly("Bob")

        val compare =
            graphStore.selectFromPool(
                ObjExprMatcher("type == 'Person' && p.name > 'B'"),
            )
    ...
    fun shouldFreezeDeepGraphVersion_withoutChangingLiveHeadUntilEdit() {
        ...
        assertThat(graphStore.write(Graph(entities = mutableListOf(liveEntity))).isValid).isTrue()
