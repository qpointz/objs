-- Policy / Category / PolicySuite catalog (C-28). PostgreSQL JSONB. Greenfield only.
-- Identity: ["key"] is the human-managed logical identity (G-P36seed); [name] is display.
-- "key" is quoted throughout for consistency with the H2 migration (reserved word there).
...
CREATE INDEX idx_objs_policy_tags_gin ON objs_policy USING GIN (tags jsonb_path_ops);
CREATE INDEX idx_objs_policy_annotations_gin ON objs_policy USING GIN (annotations jsonb_path_ops);
